...
function vcf_load(vcf_path::AbstractString)
    ...
    if length(vcf_contents) == 0
        @error "VCF Load Error: VCF data rows are empty: $vcf_path"
        return nothing
    end
    ...
    if isempty(all_formats)
        @error "VCF Load Error: VCF data rows do not contain AD: it implies no data was mapped to reference: $vcf_path"
        # close(io)
        return nothing
    end
            ...
            if nsample_alter == 0
                ...
                for i_sample in sample_indices
                    ...
                    elseif length(row_ALTs) == length(row_PROBs)
                        ...
                        @error "Bugs here. Invalid row_ALTs and row_DEPTHs" row_ALTs row_DEPTHs group nsample sample_indices df
                    end
                end
...
function compute_input_probabilities(input_vcf_all::DataFrame, db_vcf_parsed::DataFrame, groups::Vector)
    ...
    hasmissing(vcf.REF) &&
        @warn "--input does not contain all informative positions in --db-vcf"  count_all_position=nrow(db_vcf_parsed) count_missing=count(ismissing, vcf.REF)
...
$input
  ...
  Classification  = $classify_to""",
    ...
    @info "MLST variation table saved to $out_mlst_path"
    ...
    @info "Raw data (input label classify_to vcf result groups version_classifier) saved to $out_jld2_path"
...
function parsed_db_vcf_to_mlst(db_vcf_parsed::DataFrame, groups::Vector; remain_alt2probs::Bool=true)
    ...
    db_vcf_parsed = select(db_vcf_parsed, [:CHROM, :POS, :REF, :ALT2PROBs])
    for i in groups
    ...
    if remain_alt2probs
        db_vcf_parsed[!, [1:4..., group_perm...]]
    else
        db_vcf_parsed[!, [1:3..., group_perm...]]
    end
end
